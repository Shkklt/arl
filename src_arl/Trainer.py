...
class Trainer():
    ...
    def train(self):
        with open(os.path.join(self.option.this_expsdir, "train_log.txt"), "w", encoding='UTF-8') as f:
            f.write("Begin train\n")
        train_graph = Knowledge_graph(self.option, self.data_loader, self.data_loader.get_test_graph_data())
        ...
        # decay_steps = self.option.decay_batch
        ...
        for start_entities, queries, answers, all_correct in environment.get_next_batch():
            ...
            prev_state = [torch.zeros(start_entities.shape[0], self.option.relation_embed_size),
                          torch.zeros(start_entities.shape[0], self.option.relation_embed_size)]
            ...
            for step in range(self.option.max_step_length):
                ...
                # print("start_entites",start_entities[0:100])
                print("current_entities step = "+str(step),current_entities[0:20])
                print("prev_relation step = "+str(step),prev_relation[0:20])

            print("answers step = "+str(step),answers[0:20])
            print("----------------------------------")

            rewards_np = self.agent.get_reward(current_entities, answers, all_correct, self.positive_reward, self.negative_reward,prev_relation)
            ...
            torch.nn.utils.clip_grad_norm_(self.agent.parameters(), max_norm=self.option.grad_clip_norm, norm_type=5)
            self.optimizer.step()

            log.info("reinforce_loss{0:7.4f},avg_rewards{1:7.4f},batch_counter{2:4d}: ".format(reinforce_loss,np.mean(rewards_np),batch_counter))
        ...
        with torch.no_grad():
            ...
            for _start_entities, _relations, _answers, start_entities, relations, answers, all_correct\
                ...
                rewards = self.agent.get_reward(current_entities, _answers, all_correct,
                                                self.positive_reward, self.negative_reward,chosen_relation)
